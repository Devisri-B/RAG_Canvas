"""RAG retrieval layer: chunk week text -> bge embeddings -> FAISS,
with week-filtered similarity search.

``select_material()`` is wired into generation (app/generation.py): every quiz
is built from chunk -> bge -> FAISS focused material. Heavy dependencies are
imported lazily. ``WeekRetriever`` is also available for multi-week / persistent retrieval.
"""
